...
const generateWord = (optionValue) => {
  ...
  if (!options[optionValue]) {
    console.error(`Invalid option: ${optionValue}`);
    return;
  }
  ...
  if (!optionArray || optionArray.length === 0) {
    console.error(`Option array is empty for: ${optionValue}`);
    return;
  }

  //choose random word
  let randomIndex = Math.floor(Math.random() * optionArray.length);
  if (randomIndex >= optionArray.length || randomIndex < 0) {
    console.error("Random index out of bounds");
    return;
  }
  chosenWord = optionArray[randomIndex].toUpperCase();
...
// Call the initializer when the window loads
window.onload = initializer;
